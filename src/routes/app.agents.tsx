    ...
    <div className="space-y-6">
      <header>
        ...
        <p className="text-sm text-muted-foreground">
          Your AI agents, what they're doing, and the context they share via MCP.
        </p>
      </header>
          ...
          <div key={a.id} className="glass rounded-xl p-4">
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                <SourceIcon id={a.id} size={22} className="h-10 w-10 rounded-lg" />
                <div>
                  <p className="font-semibold">{a.name}</p>
                  <p className="text-xs text-muted-foreground">
                    {a.contextLevel} · {a.lastSync}
                  </p>
                </div>
              </div>
              <span
              ...
              >
                <span
                  className={`h-1.5 w-1.5 rounded-full ${a.status === "active" ? "bg-success animate-pulse" : a.status === "idle" ? "bg-muted-foreground" : "bg-danger"}`}
                />
                {a.status}
              </span>
            </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
        <div className="glass rounded-xl p-4">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Add Yantra to any AI agent that supports MCP.
          </p>
